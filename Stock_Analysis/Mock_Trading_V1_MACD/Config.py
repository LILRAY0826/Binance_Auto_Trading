...
class Function:
    ...
    def get_history_data(self, api_key: str, api_secret: str, crypto: str, start_date: str, end_date: str):
        ...
        # 輸出成csv檔
        whole_df.to_csv('/Users/rayching/Desktop/PyCharm/Stock_Analysis/Python File/計算勝率/BTCUSDT_data.csv',
                        encoding='utf-8-sig')

    # Compute DIF & MACD
    def compute_dif_and_macd(self):
        dataframe = pd.read_csv("/Users/rayching/Desktop/PyCharm/Stock_Analysis/Python File/計算勝率/BTCUSDT_data.csv", index_col=0)
        ...
        dataframe["DIF"], dataframe["MACD"], dataframe["DIF-MACD"] = talib.MACD(dataframe['close'],
                                                                                ...
                                                                                slowperiod=26,
                                                                                signalperiod=9)
        dataframe.to_csv("/Users/rayching/Desktop/PyCharm/Stock_Analysis/Python File/計算勝率/MACD.csv")

    # Compute Winning Percentage
    def compute_winning_percentage(self, start_property: int, buy: float, times: int, afford_range: int):
        # Assign Variable
        dataframe = pd.read_csv("/Users/rayching/Desktop/PyCharm/Stock_Analysis/Python File/計算勝率/MACD.csv", index_col=0)
        dif = dataframe["DIF"]
        macd = dataframe["MACD"]
        ...
        for i in range(start_index, len(dif)):
            ...
            if _direction != direction:
                ...
                if _direction == "down" and max_order_number > 0 and start_property >= buy:
                ...
                if _direction == "up" and max_order_number > 0 and start_property >= buy:
                    explode_price = round(float(open_price[i]) - float(open_price[i]) * (1 / times), 2)
                    send_price = float(open_price[i]) + afford_range
        ...
        statistical_dataframe.to_csv("/Users/rayching/Desktop/PyCharm/Stock_Analysis/Python File/計算勝率/Trading List.csv")

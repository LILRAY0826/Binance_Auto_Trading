...
class Functions:
    ...
    def true_mock_trading(self, entry_data: list, mock_trading_dataframe: pd.DataFrame, klines_dataframe: pd.DataFrame, start_property, buy, leverage_list, profit_rate_list, choppy_rate_list, days, interval, look_back_interval):
        ...
        for index, row in klines_dataframe.iterrows():
            print("{}/{}".format(index+1, len(klines_dataframe)))

            # Re-initialize position if last klines is sold.
            if long_result != None:
                long_position = [[None, None, None, None, None, None]]
            ...
            if index % look_back_interval == 0:
                ...
                # Mock Trading to compute the best parameter
                pass_klines_dataframe = self.get_pass_klines(days=days, interval=interval, end_date=open_time[:10])
                pass_entry_data = self.get_entry_time_price(pass_klines_dataframe, mock_trading_dataframe)
                ...
                for leverage in leverage_list:
                    for profit_rate in profit_rate_list:
                        for choppy_rate in choppy_rate_list:
                            end_property = self.mock_trading(entry_data=pass_entry_data,
                                                             klines_dataframe=pass_klines_dataframe,
            ...
            if long_position[0][0] == None and short_position[0][0] == None:
                leverage = max_leverage
                profit_rate = max_profit_rate
                choppy_rate = max_choppy_rate
            ...
            print("\nLeverage : {}, Profit Rate : {}, Choppy Rate : {}".format(leverage, round(profit_rate, 2), round(choppy_rate, 2)))
            print("Property : {}".format(list[0][-1]))
        ...
        try:
            return list[0][-1], round(win_number / (trading_number - short_position_accumulate - long_position_accumulate - revise_value), 2) * 100, position_history
        except:
            return list[0][-1], "No Trading", position_history
    ...
    def get_pass_klines(self, days: int, end_date, interval):
        # Get klines dataframe
        if interval == "1hr":
            interval = Client.KLINE_INTERVAL_1HOUR
            limit = 24 * days
        ...
        try:
            dataframe['Open Time'] = pd.to_datetime(dataframe['Open Time'], format="%Y-%m-%d")
        except Exception:
            dataframe['Open Time'] = pd.to_datetime(dataframe['Open Time'], format="%Y-%m-%d %H:%M:%S")

...
if __name__ == '__main__':
    ...
    progress = tqdm(total=(len(leverage_list)*len(profit_rate_list)))

    for i, leverage in enumerate(leverage_list):
        for j, profit_rate in enumerate(profit_rate_list):
            progress.update(1)
            end_property, winning_percentage, uncertain_number = functions.mock_trading(entry_data=entry_data,
    ...
    print("Leverage : {}, Profit Rate : {}".format(max_leverage, max_profit_rate))
    print("Done!")

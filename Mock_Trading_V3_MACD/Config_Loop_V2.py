...
class Functions:
    ...
    def mock_trading(self, entry_data: list, klines_dataframe: pd.DataFrame, start_property, buy, leverage, profit_rate, choppy_rate):
        ...
        try:
            return list[0][-1], round(win_number/(trading_number - short_position_accumulate - long_position_accumulate - revise_value), 2) * 100, uncertain_number, position_history
        except:
            return list[0][-1], "No Trading", uncertain_number, position_history
        ...
        for column in dataframe.columns:
            for index, item in enumerate(dataframe[column]):
                try:
                    ...
                    pass
        dataframe.to_csv("Pass Klines.csv")
